...
<div class="row">
    <div class="col-sm-6">
        <h2>Cronologías:
            <button onclick="$('#new_cronologia').show()" class="rounded-3 cyan_but"><i class='fas fa-plus'></i></button>
        </h2>
    </div>
...
$sql = "
        SELECT c.*,(
            ...
            INNER JOIN juego j2 ON j2.idJuego = jc.idJuego
            INNER JOIN mediajuego mj2 ON mj2.idJuego = j2.idJuego AND mj2.tipo = 'cover'
            WHERE jc.idCronologia = c.idCronologia
...
<div class="row">
    ...
    while ($row = $query->fetch_assoc()) {
        ...
        ?>
        <div class="item_generos col-sm-6 col-md-3 col-lg-2" id="item-<?= $row[$idname] ?>" onclick="location.href = '<?= $url ?>/cronologia/<?= $row["idCronologia"] ?>'"> 
            <div class="item_generos_content row">
                <div class="w-100">
                    ...
                    <?php } else { ?>
                        <div class="row" style="min-height: 90%;width: 90%;margin-left: 5%;">
                            <div class="col-sm-6 gen_img_slider">
                                <img src="<?= $url . "/imagenes/cover_void.png" ?>" class="w-100 " alt="...">
                            </div>
                            <div class="col-sm-6 gen_img_slider">
                                <img src="<?= $url . "/imagenes/cover_void.png" ?>" class="w-100 " alt="...">
                            </div>
...
<div class="oscur" id="new_cronologia">
    <div class="white">
        <button onclick="$('.oscur').hide()" class="close rondo2 cyan_but">
            <i class="fa fa-times"></i>
        </button>
        <form id="nueva_cronologia">
            <input type="hidden" name="funcion" value="nueva_cronologia">
            <h3>Nueva Cronologia</h3>
...
?>
<script>
    $("#nueva_cronologia").submit(function (e) {
